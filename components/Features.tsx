  ...
  return (
    <section className="features" id="features"><div className="shell">
      ...
      <div className="phone-stage product-phones five-phones">
        ...
        <article className="phone route-phone"><PhoneTop/><small className="phone-kicker">AI-POWERED ROUTE GUIDANCE</small><h3 className="screen-title">Safer Routes</h3><div className="phone-photo-map"><Image src={assetPath("/uzbekistan-safety-map-v3.png")} alt="Safety route map" fill sizes="220px"/><span className="phone-route-path"/><i className="route-start"/><i className="route-end"/><span className="map-signal light">☀</span><span className="map-signal crowd">●●</span><span className="map-signal safe">♥</span><span className="map-signal verify">✓</span></div><div className="phone-card route-summary"><b>18 min</b><small>Well-lit route</small><ul><li>3 trusted places nearby</li><li>Low incident reports</li></ul></div></article>

        <article className="phone places-phone"><PhoneTop/><h3 className="screen-title">Safe Places</h3><div className="filter-strip">{placeFilters.map((f,i)=><span className={i===3?"selected":""} key={f}>{f}</span>)}</div><div className="place-map"><Image src={assetPath("/uzbekistan-safety-map-v3.png")} alt="Filtered places map" fill sizes="220px"/><i className="place-pin">✚</i></div><div className="selected-place"><span>✚</span><div><b>Oqtepa Pharmacy</b><small>Verified · Open 24/7 · 0.6 km</small></div><em>✓</em></div><p className="phone-bottom-copy">Places that help when you need them.</p></article>

        <article className="phone scout-phone"><PhoneTop/><div className="hefi-title"><Image src={assetPath("/hearth-mascot-head.png")} alt="Hefi mascot" width={42} height={40}/><div><h3>Hefi</h3><small>HEARTH SAFETY ASSISTANT</small></div></div><div className="chat-demo"><div className="msg me">Find the safest route to Amir Temur Square.</div><div className="msg">Good lighting · High foot traffic<br/>2 hospitals · 3 trusted places<br/>Low incident activity</div><div className="msg ai-warning"><b>Route insight</b><br/>Bus 7 has repeated reports around 8 AM. Avoid today.</div></div><div className="phone-card"><b>Suggested action</b><small>Share journey with trusted contacts →</small></div></article>
